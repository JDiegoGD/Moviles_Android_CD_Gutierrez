...
fun main() {
    ...
    println("Subtotal  : S/${calcularSubtotal(carrito)}")
    println("IGV (18%) : S/${String.format("%.2f",calcularIGV(calcularSubtotal(carrito)))}")
    println("TOTAL     : S/${calcularTotal(calcularSubtotal(carrito), calcularIGV(calcularSubtotal(carrito)))}")
}